...
export const Leaderboard: FC = () => {
    ...
    const [filter, setFilter] = useState<UsersFilter>({ sort: "place", query: '', order: SortOrder.DescendingOrder });
    const sortedAndSearchedUsers = useUsers(users, filter);

    const [fetchUsers, isUsersLoading, userError] = useFetching(async (usersInPage: number, page: number) => {
    ...
    const changeSortOrder = (sort: keyof User) => {
        ...
        Object.keys(sortFields).forEach(key => sortFields[key as keyof User] = SortOrder.DescendingOrder);
    ...
    return (
        <Container sx={styles.startContainer}>
            ...
            <Container disableGutters sx={styles.horizontalContainer}>
                <ColumnHeader text='Место' width={150} changeSortOrder={changeSortOrder} sortField={"place"} sortOrder={sortFields.place} />
                <ColumnHeader text='Игрок' width={200} changeSortOrder={changeSortOrder} sortField={"name"} sortOrder={sortFields.name} />
                <ColumnHeader text='Счёт' width={200} changeSortOrder={changeSortOrder} sortField={"scores"} sortOrder={sortFields.scores} />
                <ColumnHeader text='Дата' width={200} changeSortOrder={changeSortOrder} sortField={"date"} sortOrder={sortFields.date} />
            </Container>

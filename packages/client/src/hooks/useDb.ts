...
export const useDb = <T>(
  ...
  const request = async ({ 
    ...
    const urlId = `https://bug-and-play-flappy-bird-21.ya-praktikum.tech/server/${model}/${id || ''}`
    const urlParams = `https://bug-and-play-flappy-bird-21.ya-praktikum.tech/server/${model}/${params}`

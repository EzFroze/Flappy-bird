...
import { ForumCreateThreadPage, ForumsPage, ForumThreadPage, GamePage, LeaderboardPage, NotFoundPage, PasswordPage, ProfileChangePage, ProfilePage, ServerErrorPage, SignInPage, SignUpPage } from "./pages";
  ...
  return (
    ...
    <div style={{ display: 'flex', gap: 5 }}>
      {Object.values(RoutesEnum).map((link, i) => {
        return (<div key={i}><Link to={link}>{link}</Link></div>)
      })}
    </div>
    <hr />
    <Routes>
      ...
      <Route 
        ...
        element={<RequireAuth><ProfileChangePage /></RequireAuth>} 
      />
      <Route 
